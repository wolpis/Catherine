...
@app_commands.describe(나만보기="일러스트의 가시성을 설정합니다.")
async def pixiv_search(ctx: commands.Context, tag: str, r18: bool = False, 나만보기: bool = False):
...
async def pixiv_search(ctx: commands.Context, 모드: str, 나만보기: bool = False):
    ...
    if 모드 in ['일일', '주간', '월간']:
        ...
        data = pixiv_model.request(f"/ranking.php?mode={tag_dict[모드]}&content=illust&format=json")
